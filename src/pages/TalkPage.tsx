...
const TALK_CONTROLS: TalkControl[] = [
  { id: "undo",   label: "Undo",   icon: "↩", action: "undo",   accent: "#F4B73F", labelColor: "#F5A623" },
  { id: "clear",  label: "Clear",  icon: "✕", action: "clear",  accent: "#FF7640", labelColor: "#FF6A33" },
  { id: "custom", label: "Custom", icon: "★", action: "custom", accent: "#ECD234", labelColor: "#E6C400" },
  { id: "save",   label: "Save",   icon: "💾", action: "save",  accent: "#6FC6EA", labelColor: "#5CBFEA" },
  { id: "speak",  label: "Speak!", icon: "🔊", action: "speak", accent: "#7BD531", labelColor: "#6FCF26" },
];
...
const MAIN_PAGE: TalkTile[] = [
  ...
  { id: "main-previous", label: "Previous", icon: arrowLeftIcon,  type: "nav", action: "previous" },
  ...
  { id: "main-next", label: "Next", icon: arrowRightIcon, type: "nav", action: "next" },
];
...
const makeSamplePage = (pageNumber: number): TalkTile[] => [
  ...
  { id: `page-${pageNumber}-previous`, label: "Previous", icon: arrowLeftIcon,  type: "nav", action: "previous" },
  makeSampleWord(pageNumber, 13),
  makeSampleWord(pageNumber, 14),
  makeSampleWord(pageNumber, 15),
  makeSampleWord(pageNumber, 16),
  { id: `page-${pageNumber}-next`, label: "Next", icon: arrowRightIcon, type: "nav", action: "next" },
];

const INITIAL_PAGES: TalkTile[][] = [MAIN_PAGE, makeSamplePage(2), makeSamplePage(3)];

const defaultCustomDraft = (): CustomDraft => ({ label: "", imageSrc: pizzaIcon, fileName: "" });
...
const TalkPage = () => {
  ...
  useEffect(() => {
    ...
    console.log("[TalkPage] Session started:", session.id);

    return () => {
      if (sessionIdRef.current && db) {
        endSession(db, sessionIdRef.current);
        console.log("[TalkPage] Session ended:", sessionIdRef.current);
      }
    };
  }, [ready, db]);
  ...
  const openCustomModal = () => { setCustomDraft(defaultCustomDraft()); setIsCustomModalOpen(true); };
  const closeCustomModal = () => setIsCustomModalOpen(false);
  const cancelPlacementMode = () => setPendingCustomTile(null);
  ...
  const handleCustomImageChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      setCustomDraft((prev) => ({ ...prev, imageSrc: result, fileName: file.name }));
    };
      ...
      prev.map((page, pageIndex) => {
        ...
        return page.map((tile) =>
          tile.id === targetTileId
            ? { ...newTile, id: `${newTile.id}-${currentPage}-${targetTileId}` }
            : tile
        );
      })
    );
  ...
  const handleWordTap = (tile: WordTile) => {
    ...
    console.log("[TalkPage] word tapped:", { word: tile.value, boardCellId: tile.id, page: currentPage + 1 });
  };
  ...
  const handleControlAction = (action: TalkAction, controlId: string) => {
    ...
    switch (action) {
      case "undo":   setSentenceWords((prev) => prev.slice(0, -1)); break;
      case "clear":  setSentenceWords([]); break;
      case "custom": openCustomModal(); break;
      case "speak":  speakText(displayedSentence); break;
      case "save":
      default:
        console.log("[TalkPage] control action:", { action, sentenceWords });
        break;
    }
  };
    ...
    <section className="talk-page">
      <div className="talk-board-shell">
        ...
        <div className="talk-controls">
          {TALK_CONTROLS.map((control) => {
            const style = { "--accent": control.accent, "--label-color": control.labelColor } as CSSProperties;
            return (
              <button
                key={control.id}
                type="button"
                className={`talk-control-btn ${control.action === "speak" ? "is-speak" : ""} ${lastPressedId === control.id ? "is-pressed" : ""}`}
                style={style}
        ...
        {pendingCustomTile && (
          <div className="talk-placement-bar">
            ...
            <div className="talk-placement-bar__actions">
              <button type="button" className="talk-placement-bar__btn" onClick={addCustomToCurrentPage}>Add to Current Page</button>
              <button type="button" className="talk-placement-bar__btn is-cancel" onClick={cancelPlacementMode}>Cancel</button>
            </div>
          </div>
        )}
          ...
          {currentTiles.map((tile) => {
            ...
            return (
              <button
                key={tile.id}
                type="button"
                className={`talk-tile ${isNav ? "is-nav" : "is-word"} ${lastPressedId === tile.id ? "is-pressed" : ""} ${isDisabledNav ? "is-disabled" : ""} ${pendingCustomTile && !isNav ? "is-replaceable" : ""}`}
                onClick={() => isNav ? handleNavTap(tile as NavTile) : handleWordTap(tile as WordTile)}
                aria-label={tile.label}
                aria-disabled={isDisabledNav}
                data-action={isNav ? tile.action : ""}
              >
                <span className="talk-tile__icon">
                  <img src={tile.icon} alt="" className="talk-tile__icon-img" draggable="false" />
                </span>
        ...
        <div className="talk-page-indicator" aria-label={`Page ${currentPage + 1} of ${totalPages}`}>
          <span className="talk-page-indicator__text">Page {currentPage + 1} of {totalPages}</span>
          <div className="talk-page-indicator__dots" aria-hidden="true">
            {pages.map((_, index) => (
              <span key={`page-dot-${index}`} className={`talk-page-indicator__dot ${index === currentPage ? "is-active" : ""}`} />
            ))}
          </div>
        </div>
      </div>
      ...
      {isCustomModalOpen && (
        <div className="talk-custom-modal-backdrop" onClick={closeCustomModal}>
          <div className="talk-custom-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label="Create custom tile">
            ...
            <div className="talk-custom-preview">
              <div className="talk-custom-preview__icon">
                <img src={customDraft.imageSrc || pizzaIcon} alt="" className="talk-custom-preview__img" />
              </div>
              <div className="talk-custom-preview__label">{customDraft.label.trim() || "[custom]"}</div>
            </div>
            ...
            <div className="talk-custom-modal__actions">
              <button type="button" className="talk-custom-modal__btn is-secondary" onClick={closeCustomModal}>Cancel</button>
              <button type="button" className="talk-custom-modal__btn" onClick={handleCustomDone}>Done</button>
            </div>
          </div>
        </div>
      )}
