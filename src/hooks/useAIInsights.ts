...
function generateSummaryText(
  ...
  const avgPerSession =
    totalSessions > 0 ? Math.round(totalWords / totalSessions) : totalWords;

  let text = `Over ${period}, ${totalWords} words were tapped across ${totalSessions} session${
    totalSessions !== 1 ? "s" : ""
  }, averaging ${avgPerSession} words per session.`;
...
function growthPrompt(
  ...
  return `Write 1 to 3 plain sentences comparing communication for ${period} to the previous summary. Total words are ${wordDir} by ${Math.abs(
    wordDiff
  )}. Hesitation rate has ${lagDir} by ${Math.abs(
    lagDiff
  )}%. Sentence length went from ${
    previous.avgSentenceLength
  } to ${current.avgSentenceLength} words. State only what changed, no interpretation. No markdown. No lists. No introduction.`;
}
...
function generateWordSuggestionsText(
  ...
  const smallPct =
    totalCatTaps > 0 ? Math.round((smallest.count / totalCatTaps) * 100) : 0;
...
function lagTimePrompt(
  ...
  if (previousRate === null) {
    return `Write 1 plain sentence saying there is not enough history yet to compare lag time for ${period}. State the current hesitation rate is ${currentRate}% with an average of ${(
      currentAvgMs / 1000
    ).toFixed(1)} seconds between taps.`;
  }

  const diff = currentRate - previousRate;
  const dir = diff <= 0 ? "down" : "up";

  return `Write 1 to 3 plain sentences about tap timing for ${period}. The hesitation rate is ${dir} ${Math.abs(
    diff
  )}% from last time, now at ${currentRate}%. Average time between taps is ${(
    currentAvgMs / 1000
  ).toFixed(
    1
  )} seconds. If hesitation is high suggest the board layout may need review. No markdown. No lists. No introduction.`;
}
...
export function useAIInsights(timeframe: Timeframe): UseAIInsightsResult {
  ...
  useEffect(() => {
    async function check() {
      ...
      if (db && ready) {
        const wasLoaded = getSetting<boolean>(
          db,
          "ai_model_loaded",
          PROFILE_ID
        );
  ...
  useEffect(() => {
    ...
    try {
      const stored = getSetting<AIInsight>(
        db,
  ...
  const generate = async () => {
    ...
    try {
      ...
      const totalSessions =
        (sessionResult[0]?.values?.[0]?.[0] as number | undefined) ?? 0;

      const totalWords = topWords.reduce(
        (sum: number, word: any) => sum + word.count,
        0
      );

      const totalCatTaps = categories.reduce(
      ...
      const topCatPct =
        topCat && totalCatTaps > 0
          ? Math.round((topCat.count / totalCatTaps) * 100)
          : 0;
      ...
      const prevSnapshot =
        getSetting<StatsSnapshot>(db, snapshotKey(timeframe), PROFILE_ID) ??
        null;
      ...
      const summary = generateSummaryText(
        ...
        totalWords,
        topWords,
        topCat?.category ?? "unknown",
        topCatPct,
        totalSessions
      );

      const wordSuggestions = generateWordSuggestionsText(
        categories,
        unusedWords,
        totalCatTaps
      );
      ...
      const [growth, lagTime] = await Promise.all([
        ...
        generateInsight(
          lagTimePrompt(
            ...
            hesitationRate,
            avgMs,
            prevSnapshot?.hesitationRate ?? null
          )
        ),
      ]);
